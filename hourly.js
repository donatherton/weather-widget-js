...
const hourly = {
  ...
  renderWidget(result) {
    ...
    for (let i = 0; i < 48; i++) {
      ...
      const temp = utils.convertTemp(fc[i].temp, tempUnit).toFixed(1);
      const tbg = utils.tempColour(fc[i].temp);
      const symbol = fc[i].weather[0].icon;
      const cond = fc[i].weather[0].description;
      const wndSpd = utils.convertSpd(fc[i].wind_speed, spdUnit).toFixed(0);
      const wsp = utils.wndSpdColour(fc[i].wind_speed);
      const wndDir = fc[i].wind_deg;
      const gust = utils.calcGust(fc[i].wind_gust, spdUnit);
      ...
      forecastTable +=
        ...
        ${wndSpd}${gust}${this.units.speed}</td><td>${utils.getWndDir(wndDir)}</td><td>${pres}mb</td></tr>`;
    }
